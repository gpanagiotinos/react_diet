...
const saveDataLocaly = async (food) => {
    try {
        const [saveFood, wasFoodCreated] = await dbModel.Food.findOrCreate({where: {ndbno: food.desc.ndbno}, defaults: food.desc})
        if (wasFoodCreated) {
                ...
                return previousPromise.then(() => {
                    return new Promise((resolve, reject) => {
                        dbModel.Nutrition.findOrCreate({where: {nutrient_id: nutrition.nutrient_id}, defaults: {
                                ...
                                const bulkArray = nutrition.measures.filter((measure) => {
                                    if (measure !== null) {
                                        return measure.label !== null
                                    }
                                }).map((measure) => {
                                    return {...measure, ...{foodNutritionId: data.dataValues.id}}
                                })
...
const getRandomDataFromUSDA = async () => {
    try {
        ...
        for(const item of items) {
            ...
            if (data !== undefined) {
                if (data.foods !== undefined && data.foods[0] !== undefined) {
                    const saveData = await saveDataLocaly(data.foods[0].food)
                    console.log('added')
                }
            }
        }
...
async function dbFake () {
    if (process.env.NODE_ENV === 'development') {
        ...
        //     console.log(error)
        // })
        // getRandomDataFromUSDA().then((response) => {
        //     console.log(response)
        // }).catch((error) => {
        //     console.log(error)
        // })
